...
namespace ConsoleApp4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //linq to Sql
            var retVal = from x in products where x.UnitPrice > 10 select x;
            ...
            List<int> ids = productList.SelectMany(x => x.Select(y => y.ProductID)).ToList();
            foreach (var item in ids.OrderBy(x=>x))
            {
